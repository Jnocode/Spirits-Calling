...
def preflight(do_build=False):
    ...
    # A2 音效 9 wav
    wavs = glob.glob(os.path.join(PROJ, "RawAssets", "Audio", "*.wav"))
    check("A2 音效素材", "PASS" if len(wavs) >= 9 else "FAIL",
          f"{len(wavs)}/9 wav")
    ...
    ver = "ProjectVersion" in game_txt
    check("A5 Steam 子系統設定", "PASS" if steam else "WARN",
          "DefaultEngine.ini [OnlineSubsystemSteam]" if steam else "未設定（上架接成就前要補）")
    check("A5 專案版本號", "PASS" if ver else "WARN",
          "DefaultGame.ini ProjectVersion" if ver else "未設 ProjectVersion")
